...
<script>
	...
	window.onload = function() {
		document.getElementById("checkCoAddress").addEventListener("click",
	...
	function isChar(text, type) {
		...
		if (largeChar.test(text))
			typeCount++;
		if (smallChar.test(text))
			typeCount++;
		if (num.test(text))
			typeCount++;
	...
	function isPassword() {
		document.getElementsByName("checkCoPassword")[0]
				.addEventListener(
						"keyup",
						function() {
							let password = document
									.getElementsByName("coPassword")[0];
							let isPassword = document
									.getElementsByName("checkCoPassword")[0];
...
<body onload="init()">
	<div id="background">
		<div id="top">
			<div id="accessArea">
				<span onclick="movePage('MoveCompanyLoginPage')">로그인</span> <span
					onclick="movePage('MoveCompanyJoinPage')">회원가입</span> <span
					onclick="movePage('MoveMainPage')">일반회원</span>
			</div>
			...
			<div id="mainMenuArea">
				<ul id="mainMenuList">
					<li class="mainMenu"
						onclick="moveManagerPage('MoveCheckManager',0)">의사관리</li>
					<li class="mainMenu"
						onclick="moveManagerPage('MoveCheckManager',1)">예약관리</li>
		...
		<div id="middle">
			<div id="rightArea" class="scrollBar">
				...
				<div class = "pre">◀</div>
				<div class = "name">
					상호명 <input type = "text" name = "coName" />
				</div>
				<div class = "code">
					사업자등록번호 <input type = "text" name = "coCode" />
  		  			<input type = "button" value="진위확인" onClick = "checkCoCode()" />
				</div>
				<div class = "isCheckCC">
					<!-- 비밀번호 동일한지 체크하여 보여줌 -->
				</div>
				<div class = "address">
					주소 <input type = "text" name = "coAddress" />
    				<input type = "button" value="주소검색" id = "checkCoAddress" />
				</div>
				<div class = "managerCode">
					관리자 코드 <input type="text" name = "coManagerCode" />
				</div>
				<div class = "Password">
					비밀번호 <input type = "password" name = "coPassword" onkeyup = "checkPassword()" />
				</div>
				<div class = "CheckPW">
					<!-- 비밀번호 유효성 체크하여 보여줌 -->
				</div>
				<div class = "checkPassword">
					비밀번호 확인 <input type = "password" name = "checkCoPassword" onkeyup = "isPassword()" />
				</div>
				<div class = "isCheckPW">
					<!-- 비밀번호 동일한지 체크하여 보여줌 -->
				</div>
				<div class = "phone">
					연락처 <input type = "text" name = "coPhone" />
				</div>
				<div class = "email">
					이메일 <input type = "text" name = "coEmail" />
				</div>
				<div class = "ne">▶</div>
				<div>
					<input type = "button" value = "회 원 가 입" onclick="companyJoin('CompanyJoin')" /> 
				</div>
				</form>
			</div>
		</div>

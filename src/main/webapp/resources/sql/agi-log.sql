...
-- VIEW
    ...
    -- 감성일기 피드 보기    
    CREATE OR REPLACE VIEW DDVIEW AS
    SELECT  DD_CODE AS DDCODE,
            TO_CHAR(DD_DATE, 'YYYYMMDDHH24MISS') AS DDDATE,
            DD_SUCODE AS SUCODE,
            SU_NICKNAME AS DDSUNAME,
            DD_CONTENT AS DDCONTENT,
            DD_LIKE AS DDLIKE
    FROM DD INNER JOIN SU ON DD_SUCODE = SU_CODE;
    
    -- 자유게시판 리스트 보기
	CREATE OR REPLACE VIEW FBPOSTLIST AS
	SELECT  FB_CODE AS FBCODE,
	        FB_DATE  AS FBDATE,
	        FB_SUCODE AS SUCODE,
	        SU_NICKNAME AS SUNICKNAME,
	        FB_TITLE AS FBTITLE,
	        FB_VIEW AS FBVIEW,
	        FB_LIKE AS FBLIKE
	FROM FB INNER JOIN SU ON FB_SUCODE = SU_CODE;

	-- 자유게시판 게시글 보기
	CREATE OR REPLACE VIEW FBPOSTCONTENT AS
	SELECT  FB_CODE AS FBCODE,
	        FB_DATE  AS FBDATE,
	        FB_SUCODE AS FBSUCODE,
	        SU_NICKNAME AS SUNICKNAME,
	        FB_TITLE AS FBTITLE,
